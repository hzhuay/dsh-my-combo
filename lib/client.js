...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // ── 整合包更新徽标（静默降级：路由失败/未配置仓库时不显示）──
      ctx.effect(function () {
        if (typeof document === 'undefined' || typeof fetch === 'undefined' || typeof sessionStorage === 'undefined') return function () {};
        if (sessionStorage.getItem('dsh-combo-update-dismissed') === '1') return function () {};
        var cancelled = false;
        var timer = null;
        fetch('/combo-update-check', { headers: { accept: 'application/json' } })
          .then(function (r) { return r.json().catch(function () { return null; }); })
          .then(function (data) {
            if (cancelled || !data || !data.ok || !data.updateAvailable) return;
            var el = document.createElement('div');
            el.dataset.dshUpdateNotice = '1';
            el.setAttribute('role', 'status');
            el.textContent = '整合包有更新：v' + data.current + ' → v' + data.latest + '（终端执行 dsh-update）';
            el.style.cssText = 'position:fixed;top:12px;left:50%;transform:translateX(-50%);z-index:9999;'
              + 'background:var(--dsw-alias-bg-layer-2,#222);color:var(--dsw-alias-label-primary,#eee);'
              + 'border:1px solid var(--dsw-alias-border-l2,#444);border-radius:10px;padding:8px 14px;'
              + 'font-size:13px;box-shadow:var(--dsw-shadow-lv3);cursor:pointer;max-width:min(90vw,560px)';
            el.addEventListener('click', function () {
              try { sessionStorage.setItem('dsh-combo-update-dismissed', '1'); } catch (e) {}
              el.remove();
            });
            document.body.appendChild(el);
            timer = setTimeout(function () { el.remove(); }, 15000);
          })
          .catch(function () {});
        return function () {
          cancelled = true;
          if (timer !== null) clearTimeout(timer);
          var el = document.querySelector('[data-dsh-update-notice]');
          if (el !== null) el.remove();
        };
      }, 'dsh-my-combo: update notice');
    }
